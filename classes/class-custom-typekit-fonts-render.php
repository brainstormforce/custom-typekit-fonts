...
if ( ! class_exists( 'Custom_Typekit_Fonts_Render' ) ) :
	...
	class Custom_Typekit_Fonts_Render {
		...
		function typekit_embed_head() {
			...
			if ( ! isset( $kit_list ) || empty( $kit_list ) ) {
				return;
			}
			?>
			<script>
			  (function(d) {
				var config = {
				  kitId         : '<?php echo $kit_list['kit-id']; ?>',
				  scriptTimeout : 3000,
				  async         : true
				},
		...
		function add_typekit_fonts( $custom_fonts ) {
			...
			$kit_list = get_option( 'custom-typekit-fonts' );
			$new_custom_fonts = wp_parse_args( $kit_list['kit-list'], $custom_fonts );
		...
		public function add_customizer_font_list( $value ) {
			...
			if ( isset( $kit_list['kit-list'] ) ) {
				...
				foreach ( $kit_list['kit-list'] as $font => $properties ) {
